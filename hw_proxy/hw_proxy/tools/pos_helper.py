...
from hw_proxy.core.schemas import (
    NetworkDeviceSchemas,
    SerialDeviceSchemas,
    UsbDeviceSchemas
)
from hw_proxy.core.supported_devices import (
    DevicePortType,
    DeviceType
)
...
class EscPosHelper(DeviceHelper):
    ...
    def has_fast_printer_check(self):
        """
        A quick, sub‐millisecond check of the DSR line.
        Many POS printers pull DSR low when out of paper or powered off.
        """
        result = False
        if self.has_printer():
            ser = self.printer.device
            result = ser.getDSR()
        return result

    def is_printer_ready_fast_check(self, initialized: bool = False):
        """Test if escpos printer is initialized."""
        return self.has_printer()\
            and self.get_bool_printer_status(initialized=initialized)

    def is_printer_online(self, initialized: bool = False):
        """Test if escpos printer is initialized."""
        return self.has_printer()\
            and self.get_bool_printer_status(initialized=initialized)

    def is_paper_ok(self, initialized: bool = False):
        """Test if escpos printer is initialized."""
        return self.has_printer()\
            and self.get_bool_paper_printer_status(initialized=initialized)
    ...
    def get_printer_status(self, initialized: bool = False):
        ...
        try:
            ...
            logger.error(
                "[get_printer_status] Fatal Error: "
    ...
    def get_paper_printer_status(self, initialized: bool = False):
        ...
        try:
            ...
            if self.has_printer():
                ...
                paper_status = paper_status_map.get(
                    paper_status_code,
                    "unknown"
                )
                result = {
            ...
            logger.error(
                "[get_paper_printer_status] Fatal Error: "
                "Unable to get printer status, "
                f"error: {e} "
            )
            result = {
            ...
            logger.debug(
                "[get_paper_printer_status] "
                f"Printer paper status: {paper_status}, "
            )
            if initialized is not True:
                self.close_printer()
        return result
    ...
    def get_full_printer_status(self, initialized: bool = False):
        ...
        try:
            ...
            logger.error(
                "[get_full_printer_status] Fatal Error: "
                "Unable to get printer status, "
                f"error: {e} "
            )
            result = {
    ...
    def get_bool_paper_printer_status(
        ...
        """Get Bolean representation of escpos printer status."""
        if printer_status is False:
            printer_status = self.get_paper_printer_status(
    ...
    def get_bool_full_printer_status(
        ...
        """Get Bolean representation of escpos printer status."""
        if printer_status is False:
            printer_status = self.get_full_printer_status(
                initialized=initialized
            )
        return isinstance(printer_status, dict)\
            and printer_status.get("is_online", False) is True\
    ...
    def print_dummy_receipt(self):
        """Print dummy receipt using escpos printer."""
        result = False
        try:
            self.init_printer()
            if self.has_fast_printer_check():
                ...
                result = True
                logger.debug(
                    "[print_dummy_receipt] Ticket Impress with success...")
            else:
                logger.error(
                    "[print_dummy_receipt] Printer not initialized "
                    "or not available."
                )
        except Exception as e:
            logger.error(
                "[print_dummy_receipt] Fatal Error: Unable to print receipt, "
                f"error: {e} "
            )
            self.close_printer()
            raise HwPrinterError(
    ...
    def print_receipt(self, receipt: str):
        ...
        try:
            self.init_printer()
            # self.is_printer_ready(initialized=True):
            # self.is_paper_ok(initialized=True):
            # self.has_printer():
            if self.has_fast_printer_check():
    ...
    def open_cashdrawer(self):
        ...
        try:
            ...
            if self.has_printer():
                ...
                logger.error(
                    "[open_cashdrawer] Printer not initialized "
                    "or not available."
                )
        except Exception as e:
            logger.error(
