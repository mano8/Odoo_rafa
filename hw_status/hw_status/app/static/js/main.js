...
const App = {
    ...
    API: {
        _baseUrl: 'http://192.168.1.146:9002/hw_proxy/system',
    ...
    StatusManager: {
        async updateAll() {
            ...
            let printerStatusFailed = false;
            let systemStatusFailed = false;

            try {
                const printerStatus = await App.API.getPrinterStatus();
                App.UI.connectionStatusText.textContent = printerStatus.connected ? 'Conectado' : 'Desconectado';
                App.UI.connectionStatusDot.className = `status-dot ${printerStatus.connected ? 'status-green' : 'status-red'}`;
                App.UI.printerStatusText.textContent = printerStatus.printerOnline ? 'En línea' : 'Fuera de línea';
                App.UI.printerStatusDot.className = `status-dot ${printerStatus.printerOnline ? 'status-green' : 'status-red'}`;
                if (printerStatus.paperOk) {
                ...
                App.PortInfoManager.display(printerStatus.devicePortType, printerStatus);
                App.UI.actionStatus.textContent = 'Estado de hardware actualizado.';
            } catch (error) {
                printerStatusFailed = true;
            ...
            try {
                const systemStatus = await App.API.getSystemStatus();
                App.UI.dockerStatusText.textContent = systemStatus.dockerRunning ? 'Activo' : 'Inactivo';
                App.UI.dockerStatusDot.className = `status-dot ${systemStatus.dockerRunning ? 'status-green' : 'status-red'}`;
                App.UI.hwProxyStatusText.textContent = systemStatus.hwProxyRunning ? 'Activo' : 'Inactivo';
                App.UI.hwProxyStatusDot.className = `status-dot ${systemStatus.hwProxyRunning ? 'status-green' : 'status-red'}`;
                if (!systemStatusFailed) App.UI.systemActionStatus.textContent = 'Estado de servicios actualizado.';
            } catch (error) {
